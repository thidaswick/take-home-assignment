...
import { CalendarDays, Clock, User as UserIcon, MessageCircle, ArrowLeft } from "lucide-react";
...
function TaskDetails() {
  ...
  if (isError || !task) {
    return <div className="mx-auto max-w-4xl rounded-xl border bg-card p-6">Task not found. <Link to="/tasks" className="text-primary hover:underline">Back to tasks</Link></div>;
  }
  ...
  return (
    <PageTransition>
      <div className="mx-auto max-w-4xl space-y-6">
        <Button variant="ghost" asChild className="-ml-2"><Link to="/tasks"><ArrowLeft className="mr-1 h-4 w-4" /> Back to tasks</Link></Button>
        ...
        <div className="rounded-2xl border bg-card p-6 shadow-sm">
          {editing ? (
            <form onSubmit={form.handleSubmit((values) => save.mutate(values))} className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button type="submit" disabled={save.isPending} className="gradient-brand text-white">
            ...
            <>
              ...
              <p className="mt-4 whitespace-pre-line text-sm leading-relaxed text-muted-foreground">{task.description}</p>

              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                <Meta icon={UserIcon} label="Owner" value={task.ownerName} />
                <Meta icon={CalendarDays} label="Due date" value={new Date(task.dueDate).toLocaleDateString()} />
                <Meta icon={Clock} label="Updated" value={new Date(task.updatedAt).toLocaleString()} />
              </div>
            </>
          )}
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
              ...
              {[
                ...
                <li key={i} className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{e.who} · {new Date(e.d).toLocaleString()}</p>
                  </div>
                </li>
              ))}
            </ol>
          ...
          <div className="rounded-2xl border bg-card p-6 shadow-sm">
            ...
            <p className="mt-3 text-sm text-muted-foreground">Comments will appear here once the API is wired.</p>
