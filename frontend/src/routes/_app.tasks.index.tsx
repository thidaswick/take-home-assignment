...
function TasksPage() {
  ...
  const toggleAll = () => {
    if (pageItems.every((t) => selected.has(t.id))) {
      const n = new Set(selected); pageItems.forEach((t) => n.delete(t.id)); setSelected(n);
    } else {
      const n = new Set(selected); pageItems.forEach((t) => n.add(t.id)); setSelected(n);
    }
  };
  ...
  return (
    <PageTransition>
        ...
        <div className="rounded-2xl border bg-card shadow-sm">
          ...
          {selected.size > 0 && (
            <div className="flex items-center justify-between border-b bg-primary/5 px-4 py-2 text-sm">
              ...
              <Button variant="destructive" size="sm" onClick={() => { setToDelete(Array.from(selected)); setConfirmOpen(true); }}>
          ...
          {isLoading ? (
            ...
            <EmptyState icon={ListChecks} title="No tasks match" description="Try adjusting your filters or create a new task."
              action={<Button asChild className="gradient-brand text-white"><Link to="/tasks/new">Create task</Link></Button>} />
          ) : (
                ...
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-10">
                      <Checkbox checked={pageItems.length > 0 && pageItems.every((t) => selected.has(t.id))} onCheckedChange={toggleAll} />
                    </TableHead>
          ...
          {filtered.length > 0 && (
            <div className="flex items-center justify-between border-t p-3 text-sm">
              <span className="text-muted-foreground">Page {page} of {totalPages} · {filtered.length} tasks</span>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete {toDelete.length} task{toDelete.length > 1 ? "s" : ""}?</AlertDialogTitle>
            <AlertDialogDescription>This action cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={() => del.mutate(toDelete)}>Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </PageTransition>
